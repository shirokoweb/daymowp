...
class Keyy_Commands {
	 ...
	 *
	 * @param Array $params - command parameters. Relevant keys: - 'role' (string), indicating which roles unconnected users are to be enumerated in. Can be 'all' or (on a multisite) 'super-admin' - 'page' (integer) - which page of users to do this for (from 1)
	...
	public function save_settings($data) {
		...
		$new_settings = apply_filters('keyy_save_settings_get_new_settings', array_merge($current_settings, $posted_settings), $current_settings, $posted_settings);
	 ...
	 *
	 * @return Array with the content as the 'html' value
	 */
	...
	/**
	 * Gets the connection code HTML for the front-end
	 *
	 * @return Array with the content as the 'html' value
	 */
	public function get_frontend_connection_code() {
		...
		return array(
			'html' => $this->keyy->get_frontend_connection_code()
		);
		...
		while ($keep_going) {
			...
			if (is_array($token) && isset($token['state']) && 'claimed' == $token['state'] && isset($token['user_login']) && is_string($token['user_login'])) {
				...
				if (is_a($user, 'WP_User')) {
					...
					// If a password is required, then don't immediately submit the form. This flag enables the JavaScript to understand that.
	 ...
	 * Receive the result of attempting to login
	 *
	 * @param Array $params - as described in the specification (possibly relevant keys: user_login, token, include_autologin, sso_server). Short tokens require time and home_url.
	 ...
	 */
	 public function login($params) {
		...
		if (!is_string($token_id) || '' == $token_id || strlen($token_id) > 45 || strlen($token_id) < 4) {
			return new WP_Error('invalid_token', 'The supplied token was invalid. Please try again');
		}
		
		if (strlen($token_id) < 32 && (empty($params['home_url']) || empty($params['time']))) {
			return new WP_Error('missing_parameters', 'With short tokens, you must supply time and home_url parameters', $params);
		}
		...
		if (isset($params['time'])) {
			...
			if (!is_numeric($params['time'])) {
				return new WP_Error('invalid_time', 'The provided time parameter was invalid', $params);
			}
			
			// We allow the server clock to be wrong within limits
			if (absint($params['time'] - time()) > 86400) {
			
				// A second opportunity. Perhaps the server clock is really wrong?
				$any_http_page = defined('KEYY_TIME_HTTP_URL') ? KEYY_TIME_HTTP_URL : 'https://google.com';
				
				$header = wp_remote_head($any_http_page, array('timeout' => 5));
				
				$date_header = wp_remote_retrieve_header($header, 'Date');
				
				$time_ok = false;

				if (is_string($date_header)) {
					$date = strtotime($date_header);
					if (absint($params['time'] - $date) < 86400) {
						$time_ok = true;
					}
				}
				...
				if (!$time_ok) {
					...
					error_log("Keyy: login (".$user->user_login."): inaccurate time provided (possible replay): provided=".$params['time'].", current=".time());
				
					return new WP_Error('wrong_time', __('The provided time parameter is not accurate', 'keyy'), array('params' => $params, 'server_time' => time(), 'http_time' => isset($date) ? $date : $date_header));
				}
		...
		if (isset($params['home_url'])) {
			if (!is_string($params['home_url'])) {
				return new WP_Error('invalid_url', 'The provided home_url parameter was invalid', $params);
			}
			// This is replay protection - a signed payload from the app cannot be redeployed on another site.
			if ($this->keyy->normalise_url($params['home_url']) != $this->keyy->normalise_url(home_url())) {
			
				error_log("Keyy: login (".$user->user_login."): wrong home_url provided (possible replay): provided=".$params['home_url'].", this_site=".home_url());
			
				return new WP_Error('invalid_url', 'The provided home_url parameter does not match the URL of this site', array('parameters' => $params, 'home_url' => $home_url));
			}
		}
		...
		if (isset($token['user_login']) && $token['user_login'] !== $user->user_login) {
		...
		return apply_filters('keyy_command_login', array(
			'code' => 'accepted',
			'message' => sprintf(__('The signed login request for %s was accepted.', 'keyy'), $user->user_login),
			'data' => $data,
		));
