...
class Keyy_Login {
	...
	private function get_token($token_id, $clear_cache = false) {
		...
		if (!is_string($token_id) || strlen($token_id) > 40 || strlen($token_id) < 4) return false;
	...
	/**
	 * Returns the lifetime of a login token, in seconds. Defaults to self::LOGIN_TOKEN_LIFE_TIME
	 *
	 * @return Integer
	 */
	public function get_login_token_lifetime() {
		return apply_filters('keyy_login_token_lifetime', self::LOGIN_TOKEN_LIFE_TIME);
	}
	 ...
	 *
	 * @param Array $params - If the optional key user_login is present, then the (as-yet unauthenticated) token will be 'locked' to this username; i.e. only the indicated user will be allowed to use it. If the optional key token_length is present, it is used to set the length of the token returned.
	 *
	...
	public function get_fresh_login_token($params = array()) {
		...
		$key_available = false;
		$default_token_length = $this->keyy->is_wave_enabled() ? 4 : 40;

		$token_length = empty($params['token_length']) ? $default_token_length : $params['token_length'];
		
		while (!$key_available) {
	
			// Maximum length of an option_name is 64.
			$random_id = substr(hash('sha256', SECURE_AUTH_SALT.microtime(true).rand(0, 9999).SECURE_AUTH_KEY), 0, $token_length);
		...
		$expiry_time = time() + $this->get_login_token_lifetime();
	...
	public function prune_expired_login_tokens() {
		...
		$sql = "
				...
				AND b.option_value < (UNIX_TIMESTAMP() - 86400)
			";
	...
	/**
	 * Gets the public key's fingerprint for the indicated user
	 *
	 * @param Integer $user_id 	   The WordPress user ID.
	 * @param String  $algorithm   Algorithm to use - supported is 'md5', 'sha256'
	 * @param Boolean $clear_cache Whether to clear WP's cache first.
	 *
	 * @return String|Boolean - The public key, or false for an error
	 */
	public function get_public_key_fingerprint($user_id, $algorithm = 'sha256', $clear_cache = false) {
	
		$public_key = $this->get_public_key($user_id, $clear_cache);
		
		if (false === $public_key) return $public_key;
		
		Keyy_Login_Plugin()->load_rsa_functions();
		
		$rsa = new Crypt_RSA();
		
		$rsa->loadKey($public_key);
		
		return $rsa->getPublicKeyFingerprint($algorithm);
